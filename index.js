...
function replaceText(word, replacement, text) {
  ...
  const replace = text.replaceAll(word, replacement);
  return replace;
}
...
function checkWord(word, text) {
  ...
  const found = new RegExp(word, "i");
  const result = found.test(text);
  return result;
}
...
function extractTextInParentheses(str) {
  ...
  const map = array.map((match) => match[1]);
  return map;
}
...
function findWord(str, word) {
  ...
  const search1 = str.search(regex);
  return search1;
}
